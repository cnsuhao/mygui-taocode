...
namespace MyGUI
{
	...
	void OgreRenderManager::setRenderSystem(Ogre::RenderSystem* _render)
	{
		...
		if (mRenderSystem != nullptr)
		{
			...
			{
				mVertProg = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram("MyGUI_VP", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "glsles", Ogre::GPT_VERTEX_PROGRAM);
				mVertProg->setSourceFile("MyGUI_VP.glsles");
				mVertProg->load();

				mFragProg = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram("MyGUI_FP", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "glsles", Ogre::GPT_FRAGMENT_PROGRAM);
