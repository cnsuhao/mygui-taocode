...
namespace tools
{
	...
	void FontExportSerializer::parseFont(pugi::xml_node _node)
	{
		...
		for (pugi::xpath_node_set::const_iterator code = codes.begin(); code != codes.end(); code ++)
		{
			...
			MyGUI::vector<std::string>::type values;
            MyGUI::utility::split(values, (*code).attribute().value());
			if (values.size() == 1)
	...
	void FontExportSerializer::writeFont(pugi::xml_node _parent, DataPtr _data)
	{
		...
		MyGUI::vector<std::string>::type values;
        MyGUI::utility::split(values, value, "|");
	...
	void FontExportSerializer::generateFontManualXml(MyGUI::xml::ElementPtr _root, const MyGUI::UString& _folderName, DataPtr _data)
	{
		...
		if (font != nullptr)
		{
			...
			MyGUI::vector<std::pair<MyGUI::Char, MyGUI::Char> >::type codePointRanges = font->getCodePointRanges();
			...
			for (MyGUI::vector<std::pair<MyGUI::Char, MyGUI::Char> >::const_iterator iter = codePointRanges.begin() ; iter != codePointRanges.end(); ++iter)
	...
	void FontExportSerializer::generateFont(DataPtr _data)
	{
		...
		MyGUI::vector<std::string>::type values;
        MyGUI::utility::split(values, ranges, "|");
		for (size_t index = 0; index < values.size(); index ++)
		{
			MyGUI::IntSize size = MyGUI::IntSize::parse(values[index]);
			font->addCodePointRange(size.width, size.height); // ?да
		}
