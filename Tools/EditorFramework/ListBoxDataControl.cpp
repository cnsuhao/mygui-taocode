...
namespace tools
{
	...
	void ListBoxDataControl::invalidateList()
	{
		...
		if (mParentData != nullptr)
		{
			...
			//½«skinList°´ÕÕ×ÖµäÐòÅÅÐò
			size_t sum = mListBox->getItemCount();
			for (size_t i=0;i<sum;i++)
			{
				for (size_t j=1;j<sum;j++)
				{
					if (mListBox->getItemNameAt(j) < mListBox->getItemNameAt(j-1))
					{
						mListBox->swapItemsAt(j,j-1);
					}
				}
			}
	...
	bool ListBoxDataControl::setSearchedSkin(std::string _name)
	{
	    bool findSkin = false;
		size_t selectIndex = MyGUI::ITEM_NONE;
		for (size_t index = 0; index < mListBox->getItemCount(); index ++)
		{
			std::string nameList = mListBox->getItemNameAt(index);
			if (nameList == _name)
			{
				selectIndex = index;
				findSkin = true;
		...
		if (findSkin)
		{
			mListBox->setIndexSelected(selectIndex);
			mListBox->beginToItemAt(selectIndex);


			if (mLastIndex != MyGUI::ITEM_NONE && selectIndex != MyGUI::ITEM_NONE)
			{
				if (MyGUI::InputManager::getInstance().isControlPressed())
				{
					if (mEnableChangePosition)
					{
						DataPtr data1 = *mListBox->getItemDataAt<DataPtr>(mLastIndex);
						DataPtr data2 = *mListBox->getItemDataAt<DataPtr>(selectIndex);
						eventChangePosition(data1, data2);
					}
				}
			}
			...
			DataPtr selection = selectIndex != MyGUI::ITEM_NONE ? *mListBox->getItemDataAt<DataPtr>(selectIndex) : nullptr;
			DataSelectorManager::getInstance().changeParentSelection(mParentData, selection);
		}
		return findSkin;
	}
    ...
    void ListBoxDataControl::OnDeleteData()
    {
        size_t index = mListBox->getIndexSelected();
        if (index == MyGUI::ITEM_NONE)
            return;
        ...
        int nMaxIndex = mListBox->getItemCount() - 1;
        if (nMaxIndex <= 0)
            return;
