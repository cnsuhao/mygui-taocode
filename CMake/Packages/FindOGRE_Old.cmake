...
# Then try everything else
	...
	if (MYGUI_STANDALONE_BUILD)
		...
		if(NOT $ENV{OGRE_SRC} STREQUAL "")
			set(ENV_OGRE_SRC $ENV{OGRE_SRC})
			string( REGEX REPLACE "\\\\" "/" ENV_OGRE_SRC ${ENV_OGRE_SRC} )
		endif()
		if(NOT $ENV{OGRE_HOME} STREQUAL "")
			set(ENV_OGRE_HOME $ENV{OGRE_HOME})
			string( REGEX REPLACE "\\\\" "/" ENV_OGRE_HOME ${ENV_OGRE_HOME} )
		endif()

		IF (NOT OGRE_SOURCE)
			set(OGRE_SOURCE ${ENV_OGRE_SRC} )
			set(OGRE_SOURCE ${OGRE_SOURCE} CACHE PATH "Path to Ogre sources ")
		ENDIF ()
		...
		if (NOT OGRE_BUILD)
			set(OGRE_BUILD "${ENV_OGRE_HOME}" CACHE PATH "Path to Ogre build directory")
		endif ()
		...
		SET(OGRESDK ${ENV_OGRE_HOME})
		SET(OGRESOURCE ${ENV_OGRE_SRC})
		IF (WIN32) #Windows
			...
			ELSEIF (OGRESDK)
				...
				IF (NOT OGRE_SOURCE)
					set(OGRE_SOURCE ${OGRESDK} CACHE PATH "Path to Ogre sources (set it if you don't have OGRE_HOME or OGRE_SRC environment variables)")
				ENDIF ()
			ELSEIF (OGRESOURCE)
				MESSAGE(STATUS "Using OGRE built from source")
				SET(OGRE_INCLUDE_DIR ${OGRESOURCE}/OgreMain/include ${OGRE_SOURCE}/include)
				SET(OGRE_LIB_DIR ${OGRESOURCE}/lib)
				IF (NOT OGRE_SOURCE)
					set(OGRE_SOURCE ${OGRESOURCE} CACHE PATH "Path to Ogre sources (set it if you don't have OGRE_HOME or OGRE_SRC environment variables)")
				ENDIF ()
			ENDIF ()
		...
		ELSE (WIN32) #Unix
			...
			elseif(APPLE)
				...
				IF (NOT OGRE_SOURCE)
					set(OGRE_SOURCE ${OGRESDK} CACHE PATH "Path to Ogre sources (set it if you don't have OGRE_HOME or OGRE_SRC environment variables)")
				ENDIF ()
				SET(OGRE_LIBRARIES OgreMainStatic)
				SET(OGRE_INCLUDE_DIR ${OGRE_INCLUDE_DIR} CACHE PATH "")
				SET(OGRE_LIBRARIES ${OGRE_LIBRARIES} CACHE STRING "")
				SET(OGRE_LIB_DIR ${OGRE_LIB_DIR} CACHE PATH "")
			ELSE ()
