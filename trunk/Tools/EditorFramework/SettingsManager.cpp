...
namespace tools
{
	...
	bool SettingsManager::loadSettingsFile(const std::string& _fileName)
	{
		...
		{
			// логировать если эт?ошибка формат?xml
		}
	...
	void SettingsManager::setValue(const std::string& _path, const std::string& _value)
	{
		...
		{
			MyGUI::vector<std::string>::type names;
			std::string delims("/");
			MyGUI::utility::split(names, _path, delims);

			pugi::xml_node currentNode = mUserDocument->document_element();
			for (MyGUI::vector<std::string>::const_iterator name = names.begin(); name != names.end(); name ++)
			{
	...
	void SettingsManager::mergeNodes(pugi::xml_node _nodeTarget, pugi::xml_node _nodeSource)
	{
		...
		// затираем текс??цели потому чт?любо?значение текста источник?являет? конечным
