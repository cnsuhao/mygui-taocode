...
namespace tools
{
		...
		mbIsHierarchyVisible(false)
	{
		...
		CommandManager::getInstance().getEvent("Command_ShowHierarchy")->connect(this, &MainMenuControl::CommandShowHierarchy);
	...
	void MainMenuControl::CommandShowHierarchy(const MyGUI::UString& _commandName, bool& _result)
	{
		mbIsHierarchyVisible = !mbIsHierarchyVisible;

		MyGUI::MenuItem* hierarchyMenu = mBar->findItemById("Hierarchy", true);
		if (hierarchyMenu != nullptr)
		{
			MyGUI::MenuControl* popup = hierarchyMenu->getItemChild();
			if (popup != nullptr)
			{
				std::string id = "Command_ShowHierarchy";
				for (size_t index = 0; index < popup->getItemCount(); index ++)
				{
					MyGUI::MenuItem* item = popup->getItemAt(index);
					if(item->getItemId() == id)
					{
						item->setItemChecked(mbIsHierarchyVisible);
					}
					//item->setItemChecked(item->getItemId() == id);
				}
			}
