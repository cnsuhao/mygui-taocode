...
namespace demo
{
	...
	{
		Ogre::Vector2 result; // результа?

		Ogre::Vector3 dirX = _corner1 - _corner0;
		Ogre::Vector3 dirY = _corner2 - _corner0;

		_position -= _corner0; // расстояни?от начала координа?(от точк?0)
	...
	Ogre::Vector2 SceneObject::getCoordByRel(Ogre::Vector2 _position, const Ogre::Vector2& _corner0, const Ogre::Vector2& _corner1, const Ogre::Vector2& _corner2) const
	{
		...
		if (mUScale != 1)
		{
			float count = 1 / mUScale; // колличеств?тайлов
			float x = result.x * count; // пропорцией узнаем положени?
			result.x  = x + 0.5f; // смещае?на половину, чтоб?цент?тайл?бы??середине
			result.x = fmod(result.x, 1); // отбрасывае?до за?то?получая от 0 до 1
		}

		if (mVScale != 1)
		{
			float count = 1 / mVScale; // колличеств?тайлов
			float y = result.y * count; // пропорцией узнаем положени?
			result.y  = y + 0.5f; // смещае?на половину, чтоб?цент?тайл?бы??середине
			result.y = fmod(result.y, 1); // отбрасывае?до за?то?получая от 0 до 1
		}
