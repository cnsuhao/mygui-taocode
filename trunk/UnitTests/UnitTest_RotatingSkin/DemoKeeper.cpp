...
namespace demo
{
	...
	void DemoKeeper::injectMousePress(int _absx, int _absy, MyGUI::MouseButton _id)
	{
        ...
        {
            MyGUI::IntPoint centerPos = MyGUI::IntPoint(_absx, _absy) - image->getAbsolutePosition();
            MyGUI::IntSize widgetSize = rotato->getSize();
			rotato->setCenter(MyGUI::FloatPoint((float)centerPos.left/widgetSize.width, (float)centerPos.top/widgetSize.height));
        }
	...
	{
		rotato->setAngle(atan2(-(float)_absx + rotato->getPixelCenter(false).left, (float)_absy - rotato->getPixelCenter(false).top));
