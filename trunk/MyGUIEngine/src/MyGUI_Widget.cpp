...
namespace MyGUI
{
	...
	bool Widget::_checkPoint(int _left, int _top) const
	{
        ...
        if (main != NULL && main->isType<RotatingSkin>())
        {
            MyGUI::RotatingSkin* pRotatoSkin = main->castType<RotatingSkin>();
            float fAngle = pRotatoSkin->getAngle();
            if (fAngle != 0)
            {
                fAngle *= -1;

                IntPoint selfPos = getPosition();
                _left -= selfPos.left;
                _top -= selfPos.top;

                IntPoint centerPos = pRotatoSkin->getPixelCenter();
                int relativeLeft = _left - centerPos.left;
                int relativeTop = _top - centerPos.top;

                _left = relativeLeft*cos(fAngle) - relativeTop*sin(fAngle) + centerPos.left + selfPos.left;
                _top = relativeTop*cos(fAngle) + relativeLeft*sin(fAngle) + centerPos.top + selfPos.top;
            }
        }
	...
	void Widget::setPropertyOverride(const std::string& _key, const std::string& _value)
	{
        ...
        else if (_key == "NeedCapture")
            setNeedCapture(utility::parseValue<bool>(_value));
        ...
        else if (_key == "PressEvent")
            setMousePressEvent(_value);

        else if (_key == "ReleaseEvent")
            setMouseReleaseEvent(_value);

        else if (_key == "DragBeginEvent")
            setDragBeginEvent(_value);

        else if (_key == "DragEvent")
            setDragEvent(_value);

        else if (_key == "DragEndEvent")
            setDragEndEvent(_value);

        else if (_key == "InheritsDrag")
            setInheritsDrag(utility::parseValue<bool>(_value));
    ...
    bool Widget::getInheritsDrag(void)
    {
        return mInheritsDrag;
    }

    void Widget::setInheritsDrag(bool _value)
    {
        if (mInheritsDrag == _value)
            return;

        mInheritsDrag = _value;

        if (_value)
        {
            Widget* parent = getParent();
            while (parent != nullptr && parent->getInheritsDrag())
            {
                parent = parent->getParent();
            }
            if (parent == nullptr)
            ...
            eventMouseDrag += newDelegate(parent, &Widget::onDragEvent);
            eventMouseButtonReleased += newDelegate(parent, &Widget::onDragEndEvent);
        }
        else
        {
            if (mDragEffectWidget)
            {
                eventMouseDrag -= newDelegate(mDragEffectWidget, &Widget::onDragEvent);
                eventMouseButtonReleased -= newDelegate(mDragEffectWidget, &Widget::onDragEndEvent);
            }
        }
    }

    void Widget::setMouseReleaseEvent(const std::string& _name)
    {
        mMouseReleaseEvent = _name;
        eventMouseButtonReleased += MyGUI::newDelegate(this, &Widget::onReleaseEvent);
    }

    void Widget::setMousePressEvent(const std::string& _name)
    {
        mMousePressEvent = _name;
        eventMouseButtonPressed += MyGUI::newDelegate(this, &Widget::onPressEvent);
    }

    void Widget::setDragBeginEvent(const std::string& _name)
    {
        mDragBeginEvent = _name;
    }

    void Widget::setDragEvent(const std::string& _name)
    {
        mDragEvent = _name;

        eventMouseDrag += MyGUI::newDelegate(this, &Widget::onDragEvent);
    }

    void Widget::onPressEvent(MyGUI::Widget* _sender, int _left, int _top, MyGUI::MouseButton _id)
    {
        if (_id != MouseButton::Left || _sender != this || mMousePressEvent.empty())
            return;
        if (Gui::getInstance().getScripteInterface())
        {
            Gui::getInstance().getScripteInterface()->setScriptGolbalVar(0, _left);
            Gui::getInstance().getScripteInterface()->setScriptGolbalVar(1, _top);
            Gui::getInstance().getScripteInterface()->fireUIEventHandler(mMousePressEvent, this);
        }
    }

    void Widget::onReleaseEvent(MyGUI::Widget* _sender, int _left, int _top, MyGUI::MouseButton _id)
    {
        if (_id != MouseButton::Left || _sender != this || mMouseReleaseEvent.empty())
            return;
        if (Gui::getInstance().getScripteInterface())
        {
            Gui::getInstance().getScripteInterface()->setScriptGolbalVar(0, _left);
            Gui::getInstance().getScripteInterface()->setScriptGolbalVar(1, _top);
            Gui::getInstance().getScripteInterface()->fireUIEventHandler(mMouseReleaseEvent, this);
        }
    }

    void Widget::onDragEvent(MyGUI::Widget* _sender, int _left, int _top, MyGUI::MouseButton _id)
    {
        ...
        if (Gui::getInstance().getScripteInterface())
        {
            if (!mIsDragging)
            {
                Gui::getInstance().getScripteInterface()->setScriptGolbalVar(0, _left);
                Gui::getInstance().getScripteInterface()->setScriptGolbalVar(1, _top);
                Gui::getInstance().getScripteInterface()->fireUIEventHandler(mDragBeginEvent, this);
                mIsDragging = true;
            }

            Gui::getInstance().getScripteInterface()->setScriptGolbalVar(0, _left);
            Gui::getInstance().getScripteInterface()->setScriptGolbalVar(1, _top);
            Gui::getInstance().getScripteInterface()->fireUIEventHandler(mDragEvent, this);
        }
    }

    void Widget::setDragEndEvent(const std::string& _name)
    {
        mDragEndEvent = _name;

        eventMouseButtonReleased += MyGUI::newDelegate(this, &Widget::onDragEndEvent);
    }

    void Widget::onDragEndEvent(MyGUI::Widget* _sender, int _left, int _top, MyGUI::MouseButton _id)
    {
        if (_id != MouseButton::Left)
            return;
        ...
        if (Gui::getInstance().getScripteInterface())
        {
            Gui::getInstance().getScripteInterface()->setScriptGolbalVar(0, _left);
            Gui::getInstance().getScripteInterface()->setScriptGolbalVar(1, _top);
            Gui::getInstance().getScripteInterface()->fireUIEventHandler(mDragEndEvent, this);
            mIsDragging = false;
        }
    }

    int Widget::dragDamping(int nDistance, int nDragBound)
    {
        if (nDistance <= 0)
            return nDistance;
        ...
        return  int(nDragBound * ( nDistance/ (nDistance + nDragBound - 1.0)));
    }
