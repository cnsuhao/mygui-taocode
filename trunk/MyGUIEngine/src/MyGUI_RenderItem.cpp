...
// 			else
			{
                if (mRotatingSkin == nullptr || fabs(mRotatingSkin->getAngle()) < 0.00001f)
                {
                    _target->doRender(mVertexBuffer, mTexture, mCountVertex);
                }
                else
                {
                    _target->doRotatedRender(mVertexBuffer, mTexture, mCountVertex, mRotatingSkin->getAngle(), mRotatingSkin->getRatioCenter().left, mRotatingSkin->getRatioCenter().top);
                }
			}
		}
	}
...
// проверяем только в дебаге
    ...
    void RenderItem::setRotatingSkin(RotatingSkin* _skin)
    {
        mRotatingSkin = _skin;
    }
