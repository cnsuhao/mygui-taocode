...
namespace MyGUI
{
	...
	ProgressBar::ProgressBar() :
		...
		mFillTrack(false),
        mAutoDecrease(false),
        mAutoDecreaseTime(0.0f),
        mUsedDecreaseTime(0.0f),
		mClient(nullptr)
	{
	}
	...
	void ProgressBar::setProgressAutoDecrease(bool _auto)
	{
		if (mAutoDecrease == _auto) return;
		mAutoDecrease = _auto;

		if (mAutoDecrease)
		{
			Gui::getInstance().eventFrameStart += newDelegate(this, &ProgressBar::frameDecrease);
			mRange = PROGRESS_AUTO_RANGE;
			mStartPosition = 0;
			mEndPosition = mRange;
			mAutoPosition = 0.0f;
	...
	void ProgressBar::frameDecrease(float _time)
	{
        if(!getVisible()) return;
        
		if(!mAutoDecrease) return;

		if(mAutoDecreaseTime == 0) return;

		mUsedDecreaseTime += _time * 1000;

        if (mUsedDecreaseTime > mAutoDecreaseTime)
        {
            mUsedDecreaseTime = mAutoDecreaseTime;
        }
	
		mEndPosition = (mAutoDecreaseTime - mUsedDecreaseTime) / mAutoDecreaseTime * mRange;

		updateTrack();
	}

	void ProgressBar::resetProgressAutoDecreasTime(size_t _value)
	{
		if(!mAutoDecrease) return;

		mAutoDecreaseTime = _value;
	...
	void ProgressBar::setPropertyOverride(const std::string& _key, const std::string& _value)
	{
		...
		else if (_key == "AutoDecrease")
			setProgressAutoDecrease(utility::parseValue<bool>(_value));
		else
		{
	...
	bool ProgressBar::getProgressAutoDecreas() const
	{
		return mAutoDecrease;
	}
