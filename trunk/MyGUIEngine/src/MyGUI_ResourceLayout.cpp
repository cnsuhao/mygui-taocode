...
namespace MyGUI
{
	...
	WidgetInfo ResourceLayout::parseWidget(xml::ElementEnumerator& _widget)
	{
		...
		{
			widgetInfo.intCoord = IntCoord::parse(tmp) * Gui::getInstance().getZoomScale();
	...
	Widget* ResourceLayout::createWidget(const WidgetInfo& _widgetInfo, const std::string& _prefix, Widget* _parent, bool _template)
	{
		...
		for (vector<ControllerInfo>::type::const_iterator iter = _widgetInfo.controllers.begin(); iter != _widgetInfo.controllers.end(); ++iter)
		{
