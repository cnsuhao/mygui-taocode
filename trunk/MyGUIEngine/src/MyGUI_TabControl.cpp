...
namespace MyGUI
{
	...
	TabControl::TabControl() :
        ...
        mButtonDefaultHeight(1),
		mSmoothShow(true),
		mButtonAutoWidth(true),
		mShutdown(false),
        mHeaderPlace(nullptr),
        mHeaderShelter(nullptr),
		mControls(nullptr),
		mEmpty(nullptr),
        mVLayout(false),
        mButtonSpace(0)
	...
	void TabControl::initialiseOverride()
	{
        ...
        if (isUserString("ShelterSkin"))
            mShelterSkinName = getUserString("ShelterSkin");
        ...
        assignWidget(mHeaderShelter, "HeaderShelter");
	...
	void TabControl::notifyPressedBarButtonEvent(MyGUI::Widget* _sender)
	{
		...
		for (size_t pos = 0; pos < mItemButton.size(); pos++)
		{
			...
			{
                bool bSelected = ((pos + mStartIndex) == mIndexSelect);
				// корректируем нажатость кнопки
				button->setStateSelected(bSelected);

                if (count < mItemShelter.size())
                {
                    mItemShelter[count]->setVisible(!bSelected);
                }
			}
			count ++;
		}
    ...
    void TabControl::setButtonDefaultHeight(int _height)
    {
        mButtonDefaultHeight = _height;
        if (mButtonDefaultHeight < 1)
            mButtonDefaultHeight = 1;
        
        for (size_t pos = 0; pos < mItemsInfo.size(); pos++)
        {
            mItemsInfo[pos].height = _height;
        }
    ...
    void TabControl::setButtonHeightAt(size_t _index, int _height)
    {
        MYGUI_ASSERT_RANGE(_index, mItemsInfo.size(), "TabControl::setButtonHeightAt");

        if (_height <= 0)
        {
            _height = mButtonDefaultHeight; 
        }

        mItemsInfo[_index].height = _height;
    ...
    Widget* TabControl::createShelter()
    {
        return mHeaderShelter->createWidget<Widget>(mShelterSkinName, IntCoord(), Align::Left | Align::Top);
    }

    void TabControl::_createItemShelter()
    {
        Widget* shelter = createShelter();
        shelter->setInheritsPick(true);
        mItemShelter.push_back(shelter);
    }
	...
	int TabControl::_getTextWidth(const UString& _text)
	{
        ...
        {
			_createItemButton();
            if (mHeaderShelter != nullptr && mItemShelter.empty())
                _createItemShelter();
        }
	...
	void TabControl::_insertItem(size_t _index, const UString& _name, TabItem* _sheet, Any _data)
	{
        ...
        int height = mButtonDefaultHeight;
		mWidthBar += width;

		mItemsInfo.insert(mItemsInfo.begin() + _index, TabItemInfo(width, height, _name, _sheet, _data));
	...
	void TabControl::setPropertyOverride(const std::string& _key, const std::string& _value)
	{
		...
		if (_key == "ButtonWidth")
            setButtonDefaultWidth(utility::parseValue<int>(_value) * Gui::getInstance().getZoomScale());

        else if (_key == "ButtonHeight")
            setButtonDefaultHeight(utility::parseValue<int>(_value) * Gui::getInstance().getZoomScale());
        ...
        else if (_key == "ButtonSpace")
            setButtonSpace(utility::parseValue<int>(_value) * Gui::getInstance().getZoomScale());

        else if (_key == "VLayout")
            setVLayout(utility::parseValue<bool>(_value));

        else if (_key == "ChangeSelect")
            setChangeSelect(_value);
    ...
    void TabControl::setButtonSpace(int _space)
    {
        mButtonSpace = _space;
    ...
    void TabControl::setVLayout(bool _vlayout)
    {
        mVLayout = _vlayout;
    ...
    void TabControl::setChangeSelect(const std::string& _name)
    {
        mChangeSelect = _name;

        eventTabChangeSelect += MyGUI::newDelegate(this, &TabControl::onChangeSelect);
    }

    void TabControl::onChangeSelect(MyGUI::TabControl* _sender, size_t _index)
    {
        if (Gui::getInstance().getScripteInterface())
        {
            Gui::getInstance().getScripteInterface()->setScriptGolbalVar(0, _index);
            Gui::getInstance().getScripteInterface()->fireUIEventHandler(mChangeSelect, this);
        }
    }
    ...
    void TabControl::setButtonHeight(TabItem* _item, int _height)
    {
        setButtonHeightAt(getItemIndex(_item), _height);
    }
	...
	void TabControl::updateBarOld()
	{
		...
		int width = 0;
        int height = getSize().height - mButtonDefaultHeight - mButtonSpace;
		size_t count = 0;
		...
		for (; pos < mItemsInfo.size(); pos++)
		{
            ...
            if (mVLayout)
            {
                coord.left = getSize().width - info.width;
                coord.top = height;
                coord.height = info.height;
            }

			if (coord != button->getCoord())
				button->setCoord(coord);

			width += (info.width + mButtonSpace);
            height -= (info.height + mButtonSpace);
			count ++;
		}
	...
	void TabControl::updateBarNew()
	{
		...
		// проверяем и обновляем бар
		int width = 0;
        int height = mHeaderPlace->getSize().height - mButtonDefaultHeight - mButtonSpace;
		size_t count = 0;
		size_t pos = mStartIndex;
		for (; pos < mItemsInfo.size(); pos++)
		{
            ...
            bool bSelected = (pos == mIndexSelect);
			// корректируем нажатость кнопки
			button->setStateSelected(bSelected);
            ...
            if (mVLayout)
            {
                coord.left = 0;
                coord.top = height;
                coord.width = mHeaderPlace->getWidth();
                coord.height = info.height;
            }
            ...
            if (mHeaderShelter != nullptr)
            {
                if (count >= mItemShelter.size())
                    _createItemShelter();

                Widget* shelter = mItemShelter[count];
                shelter->setVisible(!bSelected);

                IntCoord shelterCoord(width, 0, info.width, mHeaderShelter->getHeight());

                if (mVLayout)
                {
                    shelterCoord.left = 0;
                    shelterCoord.top = height;
                    shelterCoord.width = mHeaderShelter->getWidth();
                    shelterCoord.height = info.height;
                }

                if (shelterCoord != shelter->getCoord())
                    shelter->setCoord(shelterCoord);
            }
		...
		while (count < mItemButton.size())
		{
            ...
            if (count < mItemShelter.size())
            {
                mItemShelter[count]->setVisible(false);
            }
			count ++;
		}
		...
		{
			// корректируем виджет для пустоты
			mEmpty->setCoord(width, 0, mHeaderPlace->getWidth() - width - widthControls, getSize().height);
		}
