...
namespace MyGUI
{
    ...
    const unsigned long DRAG_THRESHOLD = 5; //measured in milliseconds
				...
				if (mLayerMouseFocus != nullptr)
				{
					...
					for (int i = MouseButton::Button0; i < MouseButton::MAX; ++i)
					{
						if (mMouseCapture[i])
                        {
                            ...
                            if (MouseButton::Left == i)
                            {
                                IntPoint dist = point - mLastPressed[i];
                                if((dist.left*dist.left + dist.top*dist.top) >= DRAG_THRESHOLD*DRAG_THRESHOLD)
                                    mIsDraggingLeftBtn = true;
                            }
                        }
					}
				}
	...
	bool InputManager::injectMousePress(int _absx, int _absy, MouseButton _id)
	{
		...
		if (mWidgetMouseFocus->getNeedCapture() && MouseButton::None != _id && MouseButton::MAX != _id)
		{
			// start capture
