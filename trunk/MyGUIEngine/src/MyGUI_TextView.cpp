...
namespace MyGUI
{
	...
	void TextView::update(const UString& _text, IFont* _font, int _height, Align _align, VertexColourType _format, int _maxWidth)
	{
		...
		for (; index != end; ++index)
		{
			...
			line_info.simbols.push_back(CharInfo(info->uvRect, info->texture, char_width, char_height, char_advance, char_bearingX, char_bearingY));
	...
	IntPoint TextView::getCursorPoint(size_t _position)
	{
		...
		int top = 0;
		float left = 0.0f;
		for (VectorLineInfo::const_iterator line = mLineInfo.begin();
			line != mLineInfo.end(); ++line)
		{
