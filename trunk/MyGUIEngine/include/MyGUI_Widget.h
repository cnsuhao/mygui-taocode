	...
	*/
	...
	public:
		...
		*/
		...
		void setHandler(void* handler) { mHandler = handler; }
		void* getHandler(void) const { return mHandler; }
		...
		*/
        ...
        template <typename T>
        T* getSkinWidget(const std::string& _name)
        {
            Widget* pWidget = getSkinWidget(_name);

            if (pWidget)
            {
                return pWidget->castType<T>();
            }

            return nullptr;
        }

        Widget* getSkinWidget(const std::string& _name)
        {
            Widget* pWidget = nullptr;
            assignWidget(pWidget, _name);
		...
		*/
        ...
        void setInheritsDrag(bool _value);

        bool getInheritsDrag(void);

        void setMousePressEvent(const std::string& _name);

        void setMouseReleaseEvent(const std::string& _name);

        void setDragBeginEvent(const std::string& _name);

        void setDragEvent(const std::string& _name);

        void onPressEvent(MyGUI::Widget* _sender, int _left, int _top, MyGUI::MouseButton _id);

        void onReleaseEvent(MyGUI::Widget* _sender, int _left, int _top, MyGUI::MouseButton _id);

        void onDragEvent(MyGUI::Widget* _sender, int _left, int _top, MyGUI::MouseButton _id);

        void setDragEndEvent(const std::string& _name);

        void onDragEndEvent(MyGUI::Widget* _sender, int _left, int _top, MyGUI::MouseButton _id);

        int  dragDamping(int nDistance, int nDragBound);
	...
	private:
        ...
        std::string mMousePressEvent;
        std::string mMouseReleaseEvent;
        std::string mDragBeginEvent;
        bool        mIsDragging;
        bool        mInheritsDrag;
        Widget*     mDragEffectWidget;
        std::string mDragEvent;
        std::string mDragEndEvent;
	};
