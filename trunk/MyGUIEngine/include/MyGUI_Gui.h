...
namespace MyGUI
{
	...
	public:
		virtual bool fireUIEventHandler(const std::string& strHandle, const Widget* pWidget) = 0;

        virtual bool setScriptGolbalVar(int nIndex, int nValue) = 0;
	};
	...
	public:
		...
		*/
		...
		void setScripteInterface(ScriptInterface* pInterface) { mScriptInterface = pInterface; }

		ScriptInterface* getScripteInterface(void) { return mScriptInterface; }
        
        void setZoomScale(float fScale) { mZoomScale = fScale; }
        
        float getZoomScale(void) { return mZoomScale; }
