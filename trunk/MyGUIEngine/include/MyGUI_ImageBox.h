	...
	*/
	...
	public:
		...
		*/
        ...
        void setPostAction(const std::string& _value);
		...
		*/
		...
		void setClickEvent(const std::string& _name);

		void onClickEvent(MyGUI::Widget* _sender);

        void loadImageBuff(char* _buff, int _len, int _width, int _height);

        void resortTexture();

        void setFlip(int nFlip);
	...
	private:
		...
		std::string mClickEvent;

        ITexture* mDynamicImage;

        std::string mPostAction;
	};
