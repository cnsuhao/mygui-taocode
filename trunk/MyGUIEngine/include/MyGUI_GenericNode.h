/*!
	...
	@date       08/2009
*/
#ifndef __MYGUI_GENERIC_NODE_H__
#define __MYGUI_GENERIC_NODE_H__
...
namespace MyGUI
{
	...
	{
	public:
		typedef typename vector<NODE*>::type VectorGenericNodePtr;
    ...
    template<class NODE, class OWNER>
    void GenericNode<NODE, OWNER>::remove(int nIndex)
    {
        MYGUI_DEBUG_ASSERT(nIndex >=0 && nIndex < (int)mChildren.size(), "Node Index is invalid!");

        if (mChildren[nIndex])
            delete mChildren[nIndex];

        mChildren.erase(mChildren.begin()+nIndex);
	...
	template<class NODE, class OWNER>
	inline const typename vector<NODE*>::type& GenericNode<NODE, OWNER>::getChildren() const
	{
		return mChildren;
	}
    template<class NODE, class OWNER>
    inline typename vector<NODE*>::type& GenericNode<NODE, OWNER>::getChildren()
    {
        return mChildren;
    }
    template<class NODE, class OWNER>
    ...
    template<class NODE, class OWNER>
    int GenericNode<NODE, OWNER>::getIndex() const
    {
        NODE* pParent = getParent();
        if (pParent)
        {
            for (typename VectorGenericNodePtr::iterator Iterator = pParent->mChildren.begin(); Iterator != pParent->mChildren.end(); ++Iterator)
            {
                if (*Iterator != this)
                    continue;

                return Iterator - pParent->mChildren.begin();
            }
        }
        return -1;
    }
