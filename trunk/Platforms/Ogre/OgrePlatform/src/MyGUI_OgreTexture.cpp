...
namespace MyGUI
{
    ...
    void OgreTexture::loadFromMemory(const void* _memory)
    {
        if (!mTexture.isNull())
        {
            Ogre::PixelBox pixel(getWidth(), getHeight(), mTexture->getDepth(), convertFormat(mOriginalFormat), const_cast<void*>(_memory));
            mTexture->getBuffer()->blitFromMemory(pixel);
        }
    }

    void OgreTexture::loadFromCodecMemory(const void* _memory, int _len, const std::string& _type)
    {
        if (!mTexture.isNull())
        {
            Ogre::Image image;
            Ogre::MemoryDataStreamPtr wrapper(OGRE_NEW Ogre::MemoryDataStream((void*)_memory, _len, false));

            image.load((Ogre::DataStreamPtr&)wrapper, _type);

            mTexture->getBuffer()->blitFromMemory(image.getPixelBox());
        }
    }
