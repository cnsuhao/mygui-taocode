...
namespace input
{
	...
	public:
		...
		int VirtualKeyToScanCode(unsigned int _virtualKey) const
		{
...
#ifdef INPUT_KEY_NAME
		const std::string VirtualKeyToName(unsigned int  _virtualKey) const
		{
			if (_virtualKey < VLK_MAX)
...
#ifdef INPUT_KEY_NAME
		vector<std::string>::type mVirtualKeyToName;
		vector<std::string>::type mScanCodeToName;
#endif
	};
	...
	int VirtualKeyToScanCode(unsigned int  _virtualKey)
	{
	...
	std::string VirtualKeyToName(unsigned int  _virtualKey)
	{
		const Table& table = getTable();
	...
	int VirtualKeyToText(unsigned int  _virtualKey)
	{
