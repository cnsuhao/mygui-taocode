...
namespace MyGUI
{
	...
	public:
		...
		*/
        ...
        FontDataStream* getFontDataStream(const std::string& _name);

        FontDataImpl* getFontImpl();

        void registerForPrepare(IFont* _font);
	...
	private:
        ...
        typedef map<std::string, FontDataStream*>::type FontDataStreamRegistry;
        FontDataStreamRegistry	mFontData;

        typedef set<IFont*>::type FontPrepareRegistry;
        FontPrepareRegistry mDirtyFont;

        FontDataImpl* mFontImpl;
	};
