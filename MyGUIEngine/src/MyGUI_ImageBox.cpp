...
namespace MyGUI
{
    ...
    std::string ImageBox::getImageTexture()
    {
	...
	void ImageBox::setItemResourceInfo(const ImageIndexInfo& _info)
	{
		...
		{
			vector<IntPoint>::type::const_iterator iter = _info.frames.begin();
    ...
    void ImageBox::setPostAction(const std::string& _name)
    {
        mPostAction = _name;
    }
	...
	void ImageBox::setClickEvent(const std::string& _name)
	{
        if (_name == "")
            return;
        ...
        if (mClickEvent == "")
        {
            eventMouseButtonClick += MyGUI::newDelegate(this, &ImageBox::onClickEvent);
        }
	...
	void ImageBox::onClickEvent(MyGUI::Widget* _sender)
	{
		if (Gui::getInstance().getScripteInterface())
			Gui::getInstance().getScripteInterface()->fireUIEventHandler(mClickEvent, this);
	}

    void ImageBox::loadImageBuff(char* _buff, int _len, int _width, int _height)
    {
        if (_buff == 0 || _len == 0 || _width == 0 || _height == 0) 
            return;

        if (mDynamicImage != nullptr)
        {
            RenderManager::getInstance().destroyTexture(mDynamicImage);
            mDynamicImage = nullptr;
        }

        static int textureGUID = 0;

        mDynamicImage = RenderManager::getInstance().createTexture(MyGUI::utility::toString(textureGUID, "_DynamicImage"));
        mDynamicImage->createManual(_width, _height, TextureUsage::Static | TextureUsage::Write, PixelFormat::R8G8B8A8);

        mDynamicImage->loadFromCodecMemory(_buff, _len, "jpg");
        setImageTexture(MyGUI::utility::toString(textureGUID, "_DynamicImage"));

        ++textureGUID;
    }

    void ImageBox::resortTexture()
    {
        Widget* pWidget = this;
        while(pWidget != nullptr)
        {
            if (pWidget->isRootWidget())
                break;
            
            if (pWidget->getParent() && pWidget->getParent()->isRootWidget())
                break;

            pWidget = pWidget->getParent();
        }

        MYGUI_ASSERT(pWidget != nullptr, "resortTexture error!");

        pWidget->_resortTexture();
    }

    void ImageBox::setFlip(int nFlip)
    {
        if (nFlip >= FlipNone && nFlip < FlipNum)
        {
            mFlip = nFlip;

            if (!mCurrentTextureName.empty())
            {
                setImageTexture(mCurrentTextureName);
            }

            if (!mItemName.empty())
            {
                setItemResourceInfo(mResource, mItemGroup, mItemName);
            }
        }
    }
    ...
    void ImageBox::restartAnimation(void)
    {
        frameAdvise(false);
        updateSelectIndex(0);
    }
	...
	void ImageBox::_setUVSet(const FloatRect& _rect)
	{
        ...
        case FlipHorizontal:
            {
                uvSet.left = _rect.right;
                uvSet.right = _rect.left;
            }
            break;
        case FlipVertical:
            {
                uvSet.top = _rect.bottom;
                uvSet.bottom = _rect.top;
            }
            break;
        case FlipBoth:
            {
		...
		if (nullptr != getSubWidgetMain())
			getSubWidgetMain()->_setUVSet(uvSet);
	}
	...
	void ImageBox::setPropertyOverride(const std::string& _key, const std::string& _value)
	{
		...
		/// @wproperty{ImageBox, ImageName, string} Имя картинки в группе ресурса.
			...
			setItemName(_value);
        else if (_key == "ClickEvent")
            setClickEvent(_value);
        else if (_key == "PostAction")
            setPostAction(_value);
        else if (_key == "Flip")
            setFlip(utility::parseValue<int>(_value));
		else
		{
