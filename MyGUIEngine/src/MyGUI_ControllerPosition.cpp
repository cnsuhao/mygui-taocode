...
namespace MyGUI
{
	...
	bool ControllerPosition::addTime(Widget* _widget, float _time)
	{
		...
		{
			IntCoord coord;
			eventFrameAction(mStartCoord, mDestCoord, mControlCoord, coord, mElapsedTime / mTime);
			if (mCalcPosition)
			{
		...
		IntCoord coord;
		eventFrameAction(mStartCoord, mDestCoord, mControlCoord, coord, 1.0f);
		if (mCalcPosition)
		{
			if (mCalcSize) _widget->setCoord(coord);
			else _widget->setPosition(coord.point());
    ...
    void ControllerPosition::setControlCoord(const IntCoord& _value)
    {
        mControlCoord = _value;
    }
} // namespace MyGUI
