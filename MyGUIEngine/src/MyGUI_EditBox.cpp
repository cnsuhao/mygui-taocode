...
namespace MyGUI
{
	...
	EditBox::EditBox() :
		...
		mClientText(nullptr),
        mShowDraggingTextScrollBar(false)
	{
	...
	void EditBox::notifyMousePressed(Widget* _sender, int _left, int _top, MouseButton _id)
	{
		...
		if (mModeStatic)
        {
            mViewOffsetBeforeDrag = mClientText->getViewOffset();
			return;
        }
	...
	void EditBox::notifyMouseReleased(Widget* _sender, int _left, int _top, MouseButton _id)
	{
		// сбрасываем всегда
		mMouseLeftPressed = false;
        if (isShowDraggingTextScrollBar() && isVisibleVScroll())
	...
	void EditBox::notifyMouseDrag(Widget* _sender, int _left, int _top, MouseButton _id)
	{
		...
		if (mModeStatic)
        {
            if (mVRange != 0)
            {
                if (!isVisibleVScroll() && isShowDraggingTextScrollBar())
                    setVisibleVScroll(true);

                const IntPoint& lastPressed = InputManager::getInstance().getLastPressedPosition(MouseButton::Left);
                IntPoint viewOffset = mViewOffsetBeforeDrag;
                viewOffset.top = viewOffset.top - _top + lastPressed.top;

                if (viewOffset.top < 0)
                    viewOffset.top = 0;
                else if (viewOffset.top > (int)mVRange)
                    viewOffset.top = mVRange;

                if (viewOffset != mViewOffsetBeforeDrag)
                {
                    if (mVScroll != nullptr)
                        mVScroll->setScrollPosition(viewOffset.top);
                    mClientText->setViewOffset(viewOffset);
                }
            }
            else if (mHRange != 0)
            {
                if (!isVisibleHScroll() && isShowDraggingTextScrollBar())
                    setVisibleHScroll(true);

                const IntPoint& lastPressed = InputManager::getInstance().getLastPressedPosition(MouseButton::Left);
                IntPoint viewOffset = mViewOffsetBeforeDrag;
                viewOffset.left = viewOffset.left - _left + lastPressed.left;

                if (viewOffset.left < 0)
                    viewOffset.left = 0;
                else if (viewOffset.left > (int)mHRange)
                    viewOffset.left = mVRange;

                if (viewOffset != mViewOffsetBeforeDrag)
                {
                    if (mVScroll != nullptr)
                        mVScroll->setScrollPosition(viewOffset.left);
                    mClientText->setViewOffset(viewOffset);
                }
            }
			return;
		...
		if (!mIsPressed)
		{
			...
			if (!mModeStatic)
			{
				if (mClientText != nullptr)
				{
                    ...
                    if (!mKeySetFocusEvent.empty() && Gui::getInstance().getScripteInterface())
                        Gui::getInstance().getScripteInterface()->fireUIEventHandler(mKeySetFocusEvent, this);
				}
			}
		}
	...
	void EditBox::onKeyButtonPressed(KeyCode _key, Char _char)
	{
		...
		else if ((_key == KeyCode::Return) || (_key == KeyCode::NumpadEnter))
		{
			...
			if (!mModeReadOnly)
			{
				...
				{
                    if (!mCarriageReturnEvent.empty() && Gui::getInstance().getScripteInterface())
                        Gui::getInstance().getScripteInterface()->fireUIEventHandler(mCarriageReturnEvent, this);
	...
	void EditBox::setPropertyOverride(const std::string& _key, const std::string& _value)
	{
        ...
        else if (_key == "DraggingScrBar")
            setShowDraggingTextScrollBar(utility::parseValue<bool>(_value));

        else if (_key == "KeySetFocusEvent")
            setKeySetFocusEvent(_value);

        else if (_key == "CarriageReturnEvent")
            setCarriageReturnEvent(_value);
    ...
    void EditBox::setShowDraggingTextScrollBar(bool _value)
    {
        mShowDraggingTextScrollBar = _value;
    }

    void EditBox::setKeySetFocusEvent(const std::string& _name)
    {
        mKeySetFocusEvent = _name;
    }

    void EditBox::setCarriageReturnEvent(const std::string& _name)
    {
        mCarriageReturnEvent = _name;
    }

    bool EditBox::isShowDraggingTextScrollBar() const
    {
        return mShowDraggingTextScrollBar;
    }
