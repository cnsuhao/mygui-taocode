...
namespace MyGUI
{
	...
	void RotatingSkin::setCenter(const FloatPoint& _center)
	{
		mCenterPos = _center;
		mGeometryOutdated = true;
    ...
    FloatPoint RotatingSkin::getRatioCenter() const
    {
        return mCenterPos;
    }

    IntPoint RotatingSkin::getPixelCenter(bool _local) const
	{
        IntSize widgetSize = mCroppedParent->getSize();
        IntPoint CenterPos((int)(widgetSize.width * mCenterPos.left), (int)(widgetSize.height * mCenterPos.top));
		return CenterPos + (_local ? IntPoint() : mCroppedParent->getAbsolutePosition());
	}
	...
	void RotatingSkin::createDrawItem(ITexture* _texture, ILayerNode* _node)
	{
        ...
        mRenderItem = mNode->addToRenderItem(_texture, true, true);
        mRenderItem->addDrawItem(this, VertexQuad::VertexCount);
        mRenderItem->setRotatingSkin(this);
#else
	...
	void RotatingSkin::doRender(RenderItem* _render)
	{
        ...
        float vertex_left = (float)(mCurrentCoord.left + mCroppedParent->getAbsoluteLeft() - info.leftOffset);
        float vertex_right = vertex_left + (float)mCurrentCoord.width;
        float vertex_top = (float)(mCurrentCoord.top + mCroppedParent->getAbsoluteTop() - info.topOffset);
	...
	void RotatingSkin::_rebuildGeometry()
	{
		...
		*/
		...
		float center_left = width_base * mCenterPos.left;
		float center_top = height_base * mCenterPos.top;
		...
		baseAngles[0] = atan2(center_left, center_top) + M_PI / 2;
		baseAngles[1] = atan2(- width_base + center_left, center_top) + M_PI / 2;
		baseAngles[2] = atan2(- width_base + center_left, - height_base + center_top) + M_PI / 2;
		baseAngles[3] = atan2(center_left, - height_base + center_top) + M_PI / 2;
		...
		baseDistances[0] = len(center_left, center_top);
		baseDistances[1] = len(- width_base + center_left, center_top);
		baseDistances[2] = len(- width_base + center_left, - height_base + center_top);
		baseDistances[3] = len(center_left, - height_base + center_top);
		...
		int offsetX = /*mCurrentCoord.left +*/ (int)center_left;
		int offsetY = /*mCurrentCoord.top +*/ (int)center_top;
