...
namespace MyGUI
{
	...
	{
		float fTargetZoomScale = Gui::getInstance().getZoomScale();
		fTargetZoomScale = fTargetZoomScale < 1.0f ? 1.0f : fTargetZoomScale;
		fTargetZoomScale = fTargetZoomScale > 2.0f ? 2.0f : fTargetZoomScale;
		fTargetZoomScale = ceil(fTargetZoomScale);
		...
		while (node.next(mXmlListTagName))
		{
			...
			std::string zoomscale;
			if (node->findAttribute("zoomscale", zoomscale))
			{
				if (utility::parseFloat(zoomscale) != fTargetZoomScale)
					continue;
			}
