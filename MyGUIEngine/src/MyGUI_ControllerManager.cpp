...
namespace MyGUI
{
	...
	void ControllerManager::initialise()
	{
        ...
        FactoryManager::getInstance().registerFactory<ControllerRotate>(mCategoryName);
	...
	void ControllerManager::shutdown()
	{
        ...
        FactoryManager::getInstance().unregisterFactory<ControllerRotate>(mCategoryName);
    ...
    void ControllerManager::removeItem(ControllerItem* _item)
    {
        for (ListControllerItem::iterator iter = mListItem.begin(); iter != mListItem.end(); ++iter)
        {
            if ((*iter).second == _item)
            {
                (*iter).first = nullptr;
            }
        }
